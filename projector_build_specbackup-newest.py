...
def compute(name, diag_in, ar_w, ar_h, ppi, year):
    ...
    if R > h:
        ...
        sag   = R
    t_centre  = 2.0 * sag + EDGE_THICK_MM
    od        = aperture + 2.0 * MOUNT_CLEARANCE_MM

    # ── f-number ─────────────────────────────────────────────────────────────
    f_number  = f / aperture

    # ── Housing box ──────────────────────────────────────────────────────────
    box_depth  = D_PHONE_MIRROR_MM
    box_width  = W_169 * 1.3
    box_height = H_169 * 1.3
    tube_len   = d_mirror_lens + t_centre + 20
